 ...
 *
 * @see \Ubix\Tests\Console\Command\Cron\Affiliates\ExampleOneCommandTest PHPUnit test case
 */
final class ExampleOneCommand extends CronCommand
{
    ...
    {
        $this->setDescription('The first affiliates example')
            ->setHelp(<<<'HELP'
Help for cron that is the first affiliates example
HELP);
    }
    ...
    protected function execute(Input $input, Output $output): int // phpcs:ignore SlevomatCodingStandard.Functions.UnusedParameter.UnusedParameter -- $input is required but not used
    {
        ...
        $output->writeln('AFFILIATES EXAMPLE ONE');
