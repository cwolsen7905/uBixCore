...
final class GenerateOpenapiCommand extends Command
{
    ...
    public function __construct(
        ...
        //
        //  The apps folder is resolved lazily: a framework-only checkout has no app/,
        //  and every command is instantiated at discovery, so throwing here would
        //  break `bin/ubix` as a whole rather than just this command.
        //
        $this->appsPath = $this->projectRoot->getPath('app');
    }
    ...
    protected function interact(Input $input, Output $output): void
    {
        ...
        } elseif (!is_dir($this->appsPath)) {
            $io->error('The apps folder was not found at `' . $this->appsPath . '`');
            exit(SymfonyCommand::FAILURE);
