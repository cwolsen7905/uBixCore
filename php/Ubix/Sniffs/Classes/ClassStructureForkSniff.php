...
use SlevomatCodingStandard\Sniffs\Classes\MissingClassGroupsException;
use SlevomatCodingStandard\Sniffs\Classes\UnsupportedClassGroupException;
...
class ClassStructureForkSniff implements Sniff
{
    public const CODE_INCORRECT_GROUP_ORDER        = 'IncorrectGroupOrder';
    public const CODE_INCORRECT_ALPHABETICAL_ORDER = 'IncorrectAlphabeticalOrder';
    ...
    public function process(File $phpcsFile, $stackPtr): int
    {
        ...
        $rootScopeToken     = $tokens[$stackPtr];
        ...
        while (true) {
            $nextGroup = $this->findNextGroup($phpcsFile, $groupLastMemberPtr, $rootScopeToken);
            if ($nextGroup === null) {
                break;
            }

            [$firstPtr, $lastPtr, $group] = $nextGroup;
            $groupSpans[] = [$firstPtr, $lastPtr, $group];

            // Enforce group ordering
            if ($groupsOrder[$group] >= ($groupsOrder[$expectedGroup] ?? 0)) {
                $groupsFirstMembers[$group] = $firstPtr;
                $expectedGroup              = $group;
                $groupLastMemberPtr         = $lastPtr;
                continue;
            }
            ...
            if ($fix) {
                foreach ($groupsFirstMembers as $memberGroup => $memberFirstPtr) {
                    if ($groupsOrder[$memberGroup] <= $groupsOrder[$group]) {
                        continue;
                    }
                    $this->fixIncorrectGroupOrder($phpcsFile, $firstPtr, $lastPtr, $memberFirstPtr);
                    return $stackPtr - 1;
                }
            }
        ...
        // Enforce alphabetical order within each group
        foreach ($groupSpans as [$firstPtr, $lastPtr, $group]) {
            $this->checkAlphabeticalOrder($phpcsFile, $firstPtr, $lastPtr, $rootScopeToken, $group);
        }
    ...
    /**
     * Check that all members in a group are alphabetically ordered by name.
     */
    private function checkAlphabeticalOrder(
        File $phpcsFile,
        int $groupFirstPtr,
        int $groupLastPtr,
        array $rootScopeToken,
        string $group
    ): void {
        // Determine the true start/end pointers including docblocks/whitespace
        $startPtr = $this->findGroupStartPointer($phpcsFile, $groupFirstPtr);
        $endPtr   = $this->findGroupEndPointer($phpcsFile, $groupLastPtr);

        // Gather all member definitions in this span
        $members = [];
        $pointer = $startPtr;
        while (true) {
            ...
            // Only process tokens at the class scope level
            if ($phpcsFile->getTokens()[$pointer]['level'] - $rootScopeToken['level'] !== 1) {
                continue;
            }

            if ($this->getGroupForToken($phpcsFile, $pointer) !== $group) {
                continue;
            }

            // Determine member's end pointer
            $memberEnd = $this->findGroupEndPointer($phpcsFile, $pointer);

            // Extract name for sorting
            $tokens = $phpcsFile->getTokens();
            switch ($tokens[$pointer]['code']) {
                case T_USE:
                    $raw   = $phpcsFile->getTokensAsString($pointer, $memberEnd - $pointer + 1);
                    $name  = trim(preg_replace('~;\s*$~', '', $raw));
                    break;
                case T_CONST:
                case T_ENUM_CASE:
                    $strPtr = $phpcsFile->findNext(T_STRING, $pointer + 1, $memberEnd);
                    $name   = $phpcsFile->getTokens()[$strPtr]['content'];
                    break;
                case T_VARIABLE:
                    $name = ltrim($tokens[$pointer]['content'], '$');
                    break;
                case T_FUNCTION:
            ...
            $members[] = ['name' => $name, 'start' => $pointer, 'end' => $memberEnd];
            $pointer    = $memberEnd;
        }
        ...
        if ($names !== $sorted) {
            $phpcsFile->addError(
                sprintf(
                    'Members of "%s" group are not in alphabetical order; expected: %s',
                    $group,
                    implode(', ', $sorted),
                ),
                $members[0]['start'],
                self::CODE_INCORRECT_ALPHABETICAL_ORDER
            );
            return;
        }
    }
