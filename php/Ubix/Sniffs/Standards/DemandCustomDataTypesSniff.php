...
namespace Ubix\Sniffs\Standards;
...
final class DemandCustomDataTypesSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr): void
    {
        ...
        if (!$this->isAcceptableType($returnType, true)) {
            $phpcsFile->addError(
                sprintf(
                    '%s() return type is `%s` but must be void, bool, array, an enumeration, a uBix DataType, a uBix DTO or a uBix Model',
                    $methodName,
                    $returnType,
                ),
                $stackPtr,
        ...
        foreach ($this->extractParameters($phpcsFile, $stackPtr) as $param) {
            ...
            if (!$this->isAcceptableType($typeOut, true)) {
                $phpcsFile->addError(
                    sprintf(
                        '%s() parameter %s type is `%s` but must be bool, array, an enumeration, a uBix DataType, a uBix DTO or a uBix Model',
                        $methodName,
                        $param['name'],
                        $typeOut,
                    ),
