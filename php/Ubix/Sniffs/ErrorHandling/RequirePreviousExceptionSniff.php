...
class RequirePreviousExceptionSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr): void
    {
        ...
        for ($j = $newPtr + 1; $j < $phpcsFile->numTokens; $j++) {
            ...
            // ───────────────────────────────────────────────────────────────────
            // Only grab the () that follow the exception class name.
            // The token before this "(" must be the T_STRING of the class.
        ...
        $lastComma = $phpcsFile->findPrevious(T_COMMA, $closeParen - 1, $openParen + 1, false);
        if ($lastComma === false) {
            return;
        }
        ...
        // 7) Ensure the last argument is exactly the caught variable, not an expression.
        //    Find the first non-whitespace/comment token after $lastArgPtr up to the closing paren.
        ...
        $isSimpleVar = $tokens[$lastArgPtr]['code'] === T_VARIABLE
