...
return static function (): Container {
    ...
    $container->addDefinitions([
        ...
        FilterService::class => autowire()
            ->constructorParameter('bearerToken', getenv('FILTER_API_BEARER_TOKEN')),
    ]);
...
Some words are reserved and cannot appear in the short name of a concrete class. Enforced by `tests/AbstractUbixConcreteClassOrEnumTestCase.php` via the `CLASS_NAME_RESERVED_WORDS_AND_EXEMPTIONS` constant — every concrete class runs through `testClassFollowingUbixStandards()`, which fails if the class short name contains a reserved word (case-insensitive substring) and the FQCN isn't in that word's exemption list.
...
> (`UBIX_` prefix, SCREAMING_SNAKE, explicit TTL) and never interpolate
...
final class AffiliateSqlRepository
{
    ...
    {
        $cacheKey = 'UBIX_AFFILIATE_' . $id->value;
    ...
    public function save(Affiliate $affiliate): void
    {
        ...
        // Invalidate cache
        if ($affiliate->getId() !== null) {
            $this->cache->delete('UBIX_AFFILIATE_' . $affiliate->getId()->value);
        }
    }
}
```
