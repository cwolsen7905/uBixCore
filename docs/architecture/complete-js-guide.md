...
`js/Ubix/` is a shared library that any `*Js/` app can import as `vsm`. It's wired through **npm workspaces**, declared at the repo root:
...
Running `npm install` once at the repo root installs every workspace member's deps and creates a symlink `node_modules/vsm → ../js/Ubix` (typically hoisted to the root `node_modules/`). Each consumer app declares the dep in its own `package.json`:
...
```js
import { formatCount, HoverVideo } from 'vsm';
```
...
3. **Is the visual treatment a sensible default that another consumer would either accept or override?** If yes (it's a primitive: button, card-shell, modal, video player), it belongs in `vsm`. If the visual treatment is the whole point and there's no override story (a brand-locked Header, a F4F-taxonomy filter bar), it stays per-app.

If all three answers point toward shareable: ship it in `vsm` from the start, even if there's only one consumer today. The framework grows with the project; that's how `php/Ubix/` got to its current depth.

#### Decision matrix when touching code

| Situation | Action |
|---|---|
| Adding a brand new component / utility | Apply the three questions above. Shareable → write it in `vsm` from the start. |
...
The three questions above are the rule. This table is the empirical answer for common layer types — what's emerged as belonging in `vsm` vs staying per-app as the codebase has grown. Use it to settle close calls quickly; fall back to the three questions when the layer isn't listed.
...
| **Env / config helpers** | `resolveEnvBaseUrl(env, { dev, staging, prod, fallback })` pattern that consumes `ENV` and returns a URL | Specific env-to-URL mappings per app (`{ dev: 'https://internal-admin-api.dev.vsmedia.net', … }`) | The resolution pattern repeats; the URL list is per-app. |
...
| `stores/sidebar.svelte.js` | Generalize to a `createToggleStore(initial)` factory in `vsm` |
| `chat-room/*` | WS transport + frame parsing already extracted to `vsm` (`createChatConnection`, `CHAT_COMMANDS`). Remaining components (`VideoArea`, `ChatPanel`, `SimilarRooms`) are F4F-rendering and stay per-app for now; revisit if a second consumer materializes. |
...
# `vsm` workspace symlink were set up by the root install above.
cd app/ProductJs
...
- **Every component lives in a subdir of `lib/components/`.** No flat `.svelte` files at depth 1. **Rule applies symmetrically** to both per-app code (`app/*Js/src/lib/components/`) and the shared vsm library (`js/Ubix/src/lib/components/`). Mechanical check: `find {app/*Js,js/Ubix}/src/lib/components -maxdepth 1 -name "*.svelte" -type f` must return zero.
...
const DEFAULT_API_BASE = 'https://product-api.sb.vsmedia.net';

function getApiBaseUrl() {
    if (env.PRODUCT_API_BASE_URL) return env.PRODUCT_API_BASE_URL;
    switch (env.ENV) {
        case 'PROD':    return 'https://product-api.prod.vsmedia.net';
        case 'STAGING': return 'https://product-api.staging.vsmedia.net';
        case 'DEV':     return 'https://product-api.dev.vsmedia.net';
...
#### Two-layer pattern: `createApiClient` factory (in vsm) + per-app endpoint list
...
import { createApiClient } from 'vsm';
import { env } from '$env/dynamic/private';

function getApiBaseUrl() {
    if (env.INTERNAL_ADMIN_API_BASE_URL) return env.INTERNAL_ADMIN_API_BASE_URL;
    switch (env.ENV) {
        case 'PROD':    return 'https://internal-admin-api.prod.vsmedia.net';
        case 'STAGING': return 'https://internal-admin-api.staging.vsmedia.net';
        case 'DEV':     return 'https://internal-admin-api.dev.vsmedia.net';
        default:        return 'https://internal-admin-api.sb.vsmedia.net';
    }
}
...
const WHITELABELS = [
    ...
    { pattern: /^flirt4free-com\.(dev|sb)\.vsmedia\.net$/, sitekey: 'flirt4free' },
...
```js
import { createErrorHandler, createLogger } from 'vsm';
...
window.dispatchEvent(new CustomEvent('vsm:login-modal-requested'));

// The listener, somewhere else entirely:
$effect(() => {
    if (typeof window === 'undefined') return;
    const handler = () => (loginOpen = true);
    window.addEventListener('vsm:login-modal-requested', handler);
    return () => window.removeEventListener('vsm:login-modal-requested', handler);
});
```
...
4. **A dispatch with no listener is silent.** This is not hypothetical: `vsm:get-credits-requested` / `vsm:buy-credits-requested` had **four dispatch sites and zero listeners** for months. "Get Credits" and "Buy Credits" did nothing when clicked, and the bus is precisely why nobody noticed. Shared state can't hide that the same way — the state module is one file you can grep, it says who writes it and who reads it, and an export nothing reads is a Knip finding.
...
env:
  ...
  - name: LIVE_CAMS_DEFAULT_COUNTRY
    value: "US"
  - name: F4F_ORIGIN_URL
    value: "https://flirt4free-com.sb.vsmedia.net"
```
