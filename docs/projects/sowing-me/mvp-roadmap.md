...
| M1-02 | Creator onboarding wizard (JS) + `POST /creators` (API) | M | Review | S2/S3. 2026-09-02 MR: role-on-create (`RegistrationRole`, registration TDS §3), Creator write path (`CreatorWriter`, slugExists across live+history per FR-203), `POST/GET /creator/profile` + `GET /creator/onboarding` (derived step) behind creator-role middleware, SvelteKit onboarding wizard (profile step live; tier/payout stubbed pending their surfaces). Rider: M1-03 `getBySlug` route-arg fix (PHP-DI bridge injects by name — deployed 500). |
...
| M1-04 | Tiers: spec, migration, CRUD API, creator UI | M | Review | 2026-09-02 MR: tiers/tier_benefits/subscriptions migration (active_key partial-unique per TDS §2.3), Tier/Subscription domain (query(Options), void writers), TierService (transactional reorder), TierPrecedenceService (§5), creator CRUD + public tier routes, /creator tier UI |
