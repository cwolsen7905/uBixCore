...
| Backup snapshot path fills disk on a runner host that wasn't provisioned for it | Low | `NEPTUNE_MIGRATION_BACKUP_DIR` env var lets ops point it at dedicated storage. Cron prunes after 90 days. Document in slice 6's CI wiring. |
