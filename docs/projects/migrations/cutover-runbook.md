...
1. **`dev` (`dev-db.lan.vsmedia.net`)** — first cutover; absorbs any
...
php bin/ubix migrate:status --target=staging --host=staging-db.lan.vsmedia.net --port=3306 --username=dba_user
```
...
2. **Confirm gitlab-runner host network reachability** to
   `dev-db.lan.vsmedia.net:3306` (and the equivalent staging /
   prod cluster hosts).
