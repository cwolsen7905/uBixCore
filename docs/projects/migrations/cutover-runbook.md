...
export SANDBOX_MYSQL_WRITE_DATABASE=legacy_db
...
- **Fixed (2026-06-02): `DATABASE_PREFIX` not applied to the
  ...
  failed with `SQLSTATE[HY000] [1049] Unknown database 'legacy_db'`.
  Root cause: `MigrationPdoSqlService` pinned `dbname` to the
  UNPREFIXED `MYSQL_WRITE_DATABASE` (e.g. `legacy_db`), while the
  body-apply path (`runBodyViaMysqlCli`) and the tracker table
  create/use the PREFIXED schema (`t<pid>_legacy_db`). The PDO
