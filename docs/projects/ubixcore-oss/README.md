...
| OSS-08 | `js/Ubix` → `@ubixsys/ubixcore`, `npm publish` job | Build | 2026-09-03, `feat/oss-08-npm`: package renamed + versioned 0.1.0 (BSD-3, repo metadata, README), `publish-npm` tag job → this project's GitLab npm registry with version = tag. Library is still an empty shell by design. Verify on the next tag (`v0.1.1`) |
