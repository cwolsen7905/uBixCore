...
**Starting point.** One repo (`ubixsys/ubixcore`, forked from the original monorepo)
...
now the whole standard: PSR-12 base, the PER/uBix additions, the Slevomat refs
...
trees. The two rules whose include-lists named monorepo-only files stayed in the
...
publishes every tag to the project's GitLab Composer registry (inherited).
...
- `DemandCustomDataTypes` and `ModelPropertiesType` were scoped in the old
  project file to a handful of old files that no longer exist, i.e. off.
...
**Framework.** `deploy-composer` (inherited) already publishes any `v*`
...
`js/Ubix` was a `svelte-package` shell named `ubix` 0.0.1 that exported nothing
(the old broadcasting stubs were deleted in M0-01, and `app/SowingMeJs`
...
## 2026-09-04 — migrations reach the real databases

Until today no tier database had ever been migrated: the only `migrate:up` in
any pipeline ran against the throwaway CI database, so `/creators/{slug}` was a
500 on dev, staging and prod because the tables did not exist. kitg gained a
`migrate` stage between lint-and-test and deploy: dev migrates the dev
database, staging migrates the shared staging/prod database, and main repeats
it as an idempotent no-op. Credentials come from uBixVault
`secret/kitg/db-<tier>` with a dedicated DDL user (`MYSQL_MIGRATION_*` beats
`MYSQL_WRITE_*` in the framework), destructive migrations are held on
staging/prod unless a manual `-destructive` job acknowledges them, and the
job posts its `migrate:up` summary to Discord #ops. The same stage now ships in
the skeleton's CI template with a `docs/ci-setup.md` row for the secret.

Creating the DDL user exposed a MySQL 5.6 server with a broken `procs_priv`
grant table. A ZFS snapshot made an in-place MariaDB upgrade the cheap answer;
it also dropped the binlog config, which the replica noticed first.

**Lesson worth the website:** CI proves the migration runner against a
database it creates itself. Nothing in that proves a single real database
has ever been touched. Wire the tier migrations into the pipeline on day one.

## 2026-09-04 — the last legacy names, and a second single-root history

A sweep for every `Vsm` / `Neptune` string: the phpcs sniff category became
`Ubix.Standards.*`, the required test method `testFollowingUbixStandards()`,
the backup env var `UBIX_MIGRATION_BACKUP_DIR`, and roughly 240 files of docs
and comments lost the old product and company names. The CLI trident became an
8-bit uBixSys banner. Then dev, staging and main were rewritten once more to a
single root ("uBixCore 0.1.6") so the public mirror starts from a tree with no
legacy names in its history either. Public-facing links and the skeleton's
Composer source now point at the GitHub repositories; GitLab stays an internal
