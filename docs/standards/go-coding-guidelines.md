...
[`neptune-go` base image](https://git.lan.vsmedia.net/k8s/BaseImages) (`neptune/neptune-go`),
...
The `neptune-go` base carries both tools; from an `app/*Go` dir:
...
- **CI is the authoritative run:** the `go-checks-dev` job executes
  ...
  `neptune-go` base image (same pinned tool versions the build uses). The
