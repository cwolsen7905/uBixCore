...
**All uBix Core-written Memcache keys MUST begin with `UBIX_`.** Eight characters of overhead per key, but worth it:
...
```
UBIX_<CAPABILITY>_<DESCRIPTOR>[_<DETAIL>]...
```
...
```
UBIX_CDN_PROVIDERS_ACTIVE             # CdnService eligible-list cache (this slice)
UBIX_CDN_PROVIDERS_VERSION            # version-key bust counter (sub-TTL invalidation)

UBIX_WHITELABEL_CONFIG_<DOMAIN>       # per-domain WhitelabelConfig
UBIX_WHITELABEL_VERSION               # version-key bust (REQ-WLC-RES-003)

UBIX_FEATURE_FLAG_<KEY>_<ENV>         # future Feature Flags spec REQ-FF-CACHE-*
UBIX_FEATURE_FLAG_VERSION

UBIX_RATE_LIMIT_<SCOPE>_<SUBJECT_ID>  # future Abuse Prevention REQ-RL-CACHE-*
UBIX_RATE_LIMIT_VERSION

UBIX_LIVE_MODELS_FILTER_<SERVICE>     # if/when Live Models filter chain caches
```

### 2.5 Length budget

Memcache keys are limited to **250 bytes**. With `UBIX_` accounting for 8, every key has 242 bytes of payload. This is never a real constraint for the patterns above — domain values and id substitutions stay well under the budget — but flag any key generation that could produce a value approaching 200 bytes for review.
...
1. Maintain a separate Memcache key `UBIX_<CAPABILITY>_VERSION` holding an incrementing integer.
...
uBix Core's deployment model makes cache expiry a **coordinated event**: every service runs on ≥ 5 pods sharing one Memcache, so when a hot key (e.g. `UBIX_CDN_PROVIDERS_ACTIVE`) expires, every pod misses at once and every pod hits the source DB simultaneously. For most keys that's five cheap queries and nobody notices; for an expensive query or a high-request-rate key it's a synchronized load spike. Defenses, in order of preference:
...
```
MEMCACHE_SERVERS='...,example.com:11211,example.com:9999,example.com:11211,...'
```
...
- Existing uBix Core services that follow these conventions (post-2026-05-01):
  - `Ubix\Service\Cdn\CdnService` — first slice to land using the new conventions; key `UBIX_CDN_PROVIDERS_ACTIVE`.
