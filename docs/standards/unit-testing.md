...
**Structural coverage** means the automated reflection tests in `AbstractVsmConcreteClassOrEnumTestCase` are sufficient.
...
All tests must extend `AbstractVsmConcreteClassOrEnumTestCase` and implement `UbixConcreteClassOrEnumTestCaseInterface`:
...
use Tests\AbstractVsmConcreteClassOrEnumTestCase;
 ...
 */
final class AffiliateServiceTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(AffiliateService::class);
    }
}
```

The `testFollowingVsmStandards()` method is **required** and runs automated structural validation.
...
```php
final class AffiliateSqlRepositoryTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    private static ?AffiliateSqlRepository $repository = null;

    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(AffiliateSqlRepository::class);
    }
...
```php
final class MpCodeTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(MpCode::class);
    }
...
```php
final class CreateAffiliateRequestPayloadTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(CreateAffiliateRequestPayload::class);
    }
...
```php
final class AffiliateControllerTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    private static ?AffiliateController $controller = null;

    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(AffiliateController::class);
    }
...
```php
final class AuthenticationMiddlewareTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(AuthenticationMiddleware::class);
    }
...
```php
final class ImportAffiliatesCommandTest extends AbstractVsmConcreteClassOrEnumTestCase implements UbixConcreteClassOrEnumTestCaseInterface
{
    public function testFollowingVsmStandards(): void
    {
        $this->testClassFollowingVsmStandards(ImportAffiliatesCommand::class);
    }
...
`tests/AbstractTestCase.php` reads `DATABASE_PREFIX` from env; when empty, it falls back to `tlocal_<sanitized-username>_`. Every `NeptuneDatabase::<CASE>->databaseName()` call inside production code resolves to `<prefix><DB>` so seed-data inserts, repository reads, and service tests all stay scoped to the operator's namespace.
...
- [ ] All tests include `testFollowingVsmStandards()` method
...
- **Coverage bar** — the PHP "every concrete class has a test" rule maps to **every source module has a `test_<module>.py`** with real behavioural coverage (not just an import smoke-test). New shared primitives in `vsm/` get a test in `py/Ubix/tests/`.
