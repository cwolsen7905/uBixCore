...
2. Runs the Claude CLI on the baked `ubix-claude-review` image (BaseImages) with the versioned prompt **`config/ci/claude-review-prompt.md`** — read-only tools, judgment-focused scope, explicitly told not to repeat what the gate enforces.
...
- **Model/cost:** the job pins the model + turn cap in `.gitlab-ci.yml`; the CLI is baked (deliberately unpinned) in the `ubix-claude-review` image — bump by rebuilding the BaseImages folder.
