...
php bin/ubix migrate:status --target=prod --host=prod-db.lan.vsmedia.net --port=3306
```
...
Paging is webhook-based: set `NEPTUNE_OPS_WEBHOOK_URL` as a CI variable scoped to the prod environment.
...
- `migrate-apply-{staging,prod}-destructive` — passes `--i-acknowledge-destructive` and sets `NEPTUNE_MIGRATION_BACKUP_DIR` for the snapshot (§11.4). This is the conscious destructive apply. On green it cascades to **`deploy-<tier>-destructive`** (the deploy job gated on this apply — auto on staging, manual on prod), so playing this one button carries the destructive migration *and* the deploy through.
...
```
/var/neptune-migration-backups/<migration_id>/<environment>-<timestamp>.sql.gz
```
...
**Snapshot directory must be writable by `www`.** The default root is `/var/neptune-migration-backups` (override with `NEPTUNE_MIGRATION_BACKUP_DIR`). The runtime image runs as the non-root `www` user, which **cannot** `mkdir` under `/var`. The CI `migrate-apply-*-destructive` jobs therefore set `-e NEPTUNE_MIGRATION_BACKUP_DIR=/tmp/neptune-migration-backups` — an in-container path `www` owns (mode 1777) — so the service's recursive `mkdir` of the per-migration subdir succeeds.

> **Why not a host bind mount?** The original design bind-mounted `/tmp/neptune-migration-backups → /var/...` with a job-side `mkdir -p && chmod 777`. That **breaks on the `neptune` dind runner**: the `-v` *source* resolves on the **dind daemon's** filesystem, not the job container where the `mkdir`/`chmod` ran. Docker then auto-creates the source **root-owned**, mounts it, and `www` gets `EACCES` creating the subdir — the migration fails before any DDL. Pointing `NEPTUNE_MIGRATION_BACKUP_DIR` at an in-container path sidesteps the daemon-vs-job filesystem split entirely.
>
> **Trade-off: snapshots are ephemeral under dind** — the in-container `/tmp` is discarded with the `--rm` container, so they do **not** honour the 90-day retention above. This is the already-accepted state under dind. **For durable prod snapshots**, mount a persistent volume (RWX PVC / NFS — note the runner's default PVC is RWO, which can `Multi-Attach`-fail under concurrent pods) at `/tmp/neptune-migration-backups` (or repoint the env var at the mount); no code change is needed once the volume exists. Tracked as a systems-team follow-up.
