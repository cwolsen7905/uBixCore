<script>
	...
	$: {
		...
		hasMultipleRoles =
			rolesLower.includes('creator') &&
			(rolesLower.includes('user') || rolesLower.includes('member'));
	}

	// Generate avatar URLs based on user data
	$: memberName =
		`${$userData?.user?.firstName || $userData?.firstName || ''} ${$userData?.user?.lastName || $userData?.lastName || ''}`.trim();
	...
	$: creatorAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(String(creatorName))}&background=4a90e2&color=fff`;
...
<aside class="sidebar" class:collapsed={!sidebarOpen}>
	...
	<div class="logo">
		<a href={resolve('/')}>
			...
			{:else}
				<img
					src="https://sowing.me/assets/horizontal_logo_white_small.png"
					alt="Sowing.me"
					class="logo-small"
				/>
			{/if}
	...
	<nav class="nav">
		{#each navLinks as link (link.href)}
			<a class="nav-link" href={resolve(/** @type {any} */ (link.href))}>
	...
	<div class="memberships-section">
		...
		<div class="membership-list">
			{#each memberships as membership (membership.id)}
				<a href={resolve(/** @type {any} */ (`/creator/${membership.id}`))} class="membership-item">
	...
	<div class="user-section" class:expanded={showRoleExpanded && hasMultipleRoles}>
		...
		{:else}
			...
			<div
				class="user-row"
				on:click={() => {
					if (hasMultipleRoles) showRoleExpanded = true;
				}}
			>
				<div class="user-info">
					<img class="user-avatar" src={memberAvatar} alt="Avatar" />
