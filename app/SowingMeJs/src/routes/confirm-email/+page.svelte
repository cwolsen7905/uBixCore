	...
	onMount(async () => {
		...
		try {
			const response = await fetch(
				($userData?.apiBaseUrl ?? '') + '/confirm-email?token=' + token,
				{
					method: 'GET',
					credentials: 'include'
				}
		...
		<div class="modal-card" on:click|stopPropagation role="dialog" tabindex="-1">
			...
			<div class="modal-content">
				...
				<p class="welcome-message">
					You're now able to start planting and watch other ministries grow.
				</p>
			</div>
...
{:else if errorMessage}
	<div class="error-container">
		...
		<a href={resolve('/')} class="btn-secondary">Return to Home</a>
	</div>
{/if}
