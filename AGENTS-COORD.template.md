...
you branch, before you edit a shared file, and before you merge to `main`.** It is
...
2. Create your worktree off freshly-pulled `main` (see [§2](#2-branch-rules)).
   Recommended: `php bin/ubix code:worktree <lane> <slice>` **once it is ported**
   (see note in §2); until then use the raw form:
   `git fetch origin && git worktree add ../ubixcore-worktrees/<lane> -b <prefix>/<slice> origin/main`.
...
- Branch off **freshly-pulled `main`** every time. This repo is the *framework profile*:
  `main` is the only long-lived branch, and a release is a `v*` tag, not a promotion.
  (Host repos built from the skeleton keep `dev` → `staging` → `main` — they deploy.)
- Land one slice (or one grouped logical unit) per branch **via a merge request**
  (see §4) — `main` is MR-only; never merge or push `main` directly.
...
- **Use a separate `git worktree` per agent when working concurrently.** A single
  ...
  `git worktree add ../ubixcore-worktrees/<lane> -b <prefix>/<slice> origin/main`.
  All worktrees share one `.git` and push to the same `main`, so §3 path-ownership
  ...
  > **Note:** the `php bin/ubix code:worktree` bootstrap is **not
...
`main` is **MR-only** — every landing goes through a merge request, per
[the standard's One land path](docs/standards/branching-and-git-workflow.md#one-land-path-mr-only-dev-2026-07-30).
Never `git merge` into or `git push` `main` yourself.

Protocol:
1. **Announce** in §6: `LANDING <branch> (MR)` before you push.
2. Make the branch current on YOUR branch: `git fetch origin && git rebase origin/main`.
3. Push + open the MR in one step:
   `git push -o merge_request.create -o merge_request.target=main origin <your-branch>`.
...
5. A human approves and merges. A conflict against moved `main` is resolved on YOUR
